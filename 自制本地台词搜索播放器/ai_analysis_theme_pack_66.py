...
PALETTES_66: Dict[str, PaletteSpec] = {
    ...
    "哲风玻璃": PaletteSpec("哲风玻璃", "#121212", "#2B2B31", "#6C63FF", "#E8E2D9", "#A0A0A0", "#4B5563", "rgba(255,255,255,0.08)"),
...
def build_qss(theme_name: str, palette_name: Optional[str] = None, background_image: str = "") -> str:
    ...
    bg_rule = f"background-image:url('{background_image}'); background-position:center; background-repeat:no-repeat;" if background_image else ""
    main_bg = f"""
    ...
    QFrame#softCard, QGroupBox#softCard {{
        ...
        border: 1px solid {palette.border};
        border-radius: {max(14, theme.radius-8)}px;
        padding: 8px;
    }}
    """
    nav = f"""
    QTabBar::tab {{
        ...
        color: {palette.text};
        border: 1px solid {palette.border};
        border-radius: {max(14, theme.radius-10)}px;
        padding: 10px 18px;
        margin-right: 8px;
    }}
    ...
    QPushButton {{
        ...
        color: {palette.bg};
        border: 1px solid {palette.border};
        border-radius: {max(12, theme.radius-12)}px;
        padding: 8px 16px;
        font-weight: 700;
    }}
    QPushButton:hover {{
